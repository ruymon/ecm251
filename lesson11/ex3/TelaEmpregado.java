...
public class TelaEmpregado extends JFrame implements ActionListener {
    ...
    public TelaEmpregado() {
        ...
        JLabel instrucao = new JLabel("Preencha apenas um grupo de campos conforme o tipo de empregado.", JLabel.CENTER);
    ...
    private Empregado criarEmpregado() {
        ...
        if (grupos > 1) {
            throw new IllegalArgumentException("Preencha apenas um grupo de campos.");
        }

        if (temMensalista) {
            return new Mensalista(n, m, Double.parseDouble(salarioMensal.getText().trim()));
        }
        if (temComissionado) {
            return new Comissionado(n, m,
